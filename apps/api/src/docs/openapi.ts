...
paths:
  ...
  /reports/analytics:
    get: { summary: Analytics report suite with trend, account history, category drill-down, liquidity forecast, investment performance (net worth includes a per-account cash breakdown for the asset-distribution chart) and a holding-comparison dataset (per-instrument unit price and per-account total holding value over time), parameters: [{ in: query, name: from, schema: { type: string, format: date-time } }, { in: query, name: to, schema: { type: string, format: date-time } }, { in: query, name: accountIds, description: Comma-separated account UUIDs; omit for all or use none for no accounts, schema: { type: string } }, { in: query, name: granularity, schema: { type: string, enum: [day, week, month, year] } }], responses: { '200': { description: Analytics datasets }, '422': { description: Invalid range or account filter } } }
