...
const scheduledSeries=(await request("/api/v1/scheduled-transactions",{
  method:"POST",headers:authorization,
  body:JSON.stringify({bookId:book.id,accountId:account.id,transactionType:"EXPENSE",categoryId:category.id,title:"Monthly smoke payment",amount:"50",currencyCode:"TRY",scheduledAt:"2026-09-10T12:00:00.000Z",recurrence:{frequency:"MONTHLY",interval:1,until:"2026-11-10T12:00:00.000Z"}})
})).payload;
...
const realized=(await request(`/api/v1/scheduled-transactions/${occurrences[0].id}/realize`,{
  method:"POST",headers:authorization,
  body:JSON.stringify({version:occurrences[0].version,transactionDate:"2026-09-10T12:00:00.000Z",clientOperationId:randomUUID()})
})).payload;
...
if(!transactionsAfterRealization.items.some(item=>item.id===realized.transaction.id&&item.title==="Monthly smoke payment"))throw new Error("Realized plan was not visible in transactions");
...
const carriedLedger=(await request(`/api/v1/transactions?bookId=${book.id}&accountIds=${account.id}&from=2026-08-08T00:00:00.000Z&to=2026-08-31T23:59:59.999Z&limit=1000`,{headers:authorization})).payload;
...
if(Number(balanceAfterSale.balance)!==1304.5)throw new Error(`Sale proceeds did not reach destination account: ${balanceAfterSale.balance}`);
...
if(Number(balanceAfterSaleUpdate.balance)!==1214.5)throw new Error(`Updated sale did not replace account proceeds: ${balanceAfterSaleUpdate.balance}`);
...
if(salesAfterDelete.items.length!==0||Number(balanceAfterSaleDelete.balance)!==824.5||positionAfterSaleDelete.items[0]?.quantity!=="10.00000000")throw new Error("Deleted sale did not restore portfolio and account balance");
