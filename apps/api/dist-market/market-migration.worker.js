...
// scripts/market-migration.worker.ts
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
// scripts/market-migration.worker.ts
import migration from "./dd24c90dee43b9952fd0cef96d4067b843123544-016_market_data_automation.sql";
async function withClient(env2, action) {
  const client = new esm_default.Client({ connectionString: env2.HYPERDRIVE.connectionString });
...
var market_migration_worker_default = {
  async fetch(request, env2) {
    const url = new URL(request.url);
    try {
      if (request.method === "GET" && url.pathname === "/status") {
        const value = await withClient(env2, async (client) => {
          const result = await client.query(
            `SELECT EXISTS(SELECT 1 FROM schema_migrations WHERE filename='016_market_data_automation.sql') AS applied,
                    count(*)::int AS count FROM schema_migrations`
          );
      ...
      if (request.method === "POST" && url.pathname === "/migrate") {
        const value = await withClient(env2, async (client) => {
          const exists2 = await client.query(`SELECT 1 FROM schema_migrations WHERE filename='016_market_data_automation.sql'`);
          if (exists2.rowCount) return { applied: false, alreadyApplied: true };
          await client.query("BEGIN");
          try {
            await client.query(migration);
            await client.query(`INSERT INTO schema_migrations(filename) VALUES('016_market_data_automation.sql')`);
      ...
      return Response.json({ error: error3 instanceof Error ? error3.message : "Migration failed" }, { status: 500 });
    }
  }
};
export {
  market_migration_worker_default as default
};
//# sourceMappingURL=market-migration.worker.js.map
